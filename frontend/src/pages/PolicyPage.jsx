...
export default function PolicyPage() {
  ...
  const riskConfig = {
    CRITICAL: {
      ...
      dot: "bg-red-500",
      severity: t("text-red-400", "text-red-600"),
    },
    HIGH: {
      ...
      dot: "bg-orange-500",
      severity: t("text-orange-400", "text-orange-600"),
    },
    MEDIUM: {
      ...
      dot: "bg-yellow-500",
      severity: t("text-yellow-400", "text-yellow-600"),
    },
    LOW: {
      ...
      dot: "bg-emerald-500",
      severity: t("text-emerald-400", "text-emerald-600"),
    },
  };
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className={card}>
            ...
            <div className="p-4">
              <textarea
                rows={16}
                className={`w-full px-4 py-3 rounded-xl border text-sm font-mono leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/60 transition-all duration-200 ${t("bg-gray-950 border-gray-700 text-gray-100 placeholder-gray-600 focus:border-indigo-500/50", "bg-gray-50 border-gray-200 text-gray-900 focus:border-indigo-400")}`}
                value={policyText}
          ...
          <div className={card}>
            <div className="p-4 space-y-4">
              <div className="flex flex-col gap-1">
                <label className={`text-[10px] font-bold uppercase tracking-wider ${t("text-gray-500", "text-gray-400")}`}>
                  Cloud Provider
                </label>
                <select
              ...
              <button
                ...
                disabled={loading}
                className={`w-full flex items-center justify-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl transition-all duration-200 shadow-md ${t("shadow-indigo-900/40", "shadow-indigo-200")}`}
              >
                {loading ? (
                  <>
          ...
          {loading && (
            <div className={`${card} p-5 space-y-3`}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-xs font-bold uppercase tracking-wider ${t("text-indigo-400", "text-indigo-500")}`}>Running analysis...</span>
              </div>
        ...
        <div className="lg:col-span-7 space-y-5">
          ...
          {!result && !loading && (
            <div className={`${card} flex flex-col items-center justify-center py-20 px-8 text-center`}>
              <div className={`w-14 h-14 rounded-2xl border flex items-center justify-center mb-5 ${t("bg-gray-800 border-gray-700", "bg-gray-100 border-gray-200")}`}>
                <svg className={`w-7 h-7 ${t("text-gray-600", "text-gray-400")}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                </svg>
              </div>
              <p className={`text-sm font-semibold ${t("text-gray-500", "text-gray-400")}`}>No analysis yet</p>
              <p className={`text-xs mt-1.5 leading-relaxed max-w-xs ${t("text-gray-600", "text-gray-400")}`}>
                Paste your IAM policy on the left and click Analyze Policy to see findings, risk level, and remediation suggestions.
              </p>
            </div>
          )}
          ...
          {result && (
            <>
              {/* ── Risk Level Banner ── */}
              <div className={`rounded-2xl border p-5 flex items-center gap-4 ${risk.bg} ${risk.border}`}>
                <div className={`w-3 h-3 rounded-full flex-shrink-0 shadow-lg ${risk.dot}`}></div>
                <div className="flex-1 min-w-0">
                  <p className={`text-[10px] font-bold uppercase tracking-widest ${t("text-gray-500", "text-gray-400")}`}>Overall Risk Level</p>
                  <p className={`text-xl font-black tracking-tight mt-0.5 ${t("text-white", "text-gray-900")}`}>{result.risk_level}</p>
                </div>
              ...
              {result.findings?.length > 0 && (
                <div className={card}>
                  <div className={cardHeader}>
                    <svg className={`w-4 h-4 ${t("text-amber-400", "text-amber-500")}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                    </svg>
                    <span className={`text-xs font-semibold uppercase tracking-wider ${t("text-gray-400", "text-gray-500")}`}>Findings</span>
                    ...
                    {result.findings.map((f, idx) => {
                      ...
                      return (
                        <div key={idx} className={`rounded-xl border overflow-hidden ${t("border-gray-700/60", "border-gray-200")}`}>
                          {/* Finding header */}
                          <div className={`px-4 py-3 flex items-center justify-between gap-3 ${t("bg-gray-800/50", "bg-gray-50")}`}>
                          ...
                          <div className="px-4 py-3 space-y-3">
                            <p className={`text-sm leading-relaxed whitespace-pre-wrap ${t("text-gray-400", "text-gray-600")}`}>
                              {f.explanation}
                            </p>

                            {f.sources?.length > 0 && (
                              <div className={`pt-3 border-t space-y-1.5 ${t("border-gray-700/60", "border-gray-100")}`}>
                                <p className={`text-[10px] font-bold uppercase tracking-wider ${t("text-gray-600", "text-gray-400")}`}>References</p>
                                {[...new Map(f.sources.map(s => [s.source, s])).values()].map((s, i) => (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className={`flex items-center gap-2 text-xs font-semibold transition-colors ${t("text-indigo-400 hover:text-indigo-300", "text-indigo-600 hover:text-indigo-700")}`}
                                  >
              ...
              {result.suggested_policy && (
                <div className={card}>
                  <div className={cardHeader}>
                    <svg className={`w-4 h-4 ${t("text-emerald-400", "text-emerald-500")}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className={`text-xs font-semibold uppercase tracking-wider ${t("text-gray-400", "text-gray-500")}`}>Suggested Policy</span>
                  </div>
                  <div className="p-4">
                    <pre className={`text-xs font-mono leading-relaxed rounded-xl p-4 overflow-auto max-h-80 ${t("bg-gray-950 text-emerald-300 border border-gray-800", "bg-gray-950 text-emerald-300 border border-gray-800")}`}>
              ...
              {result.policy_diff && (
                <div className={card}>
                  <div className={cardHeader}>
                    ...
                    <span className={`text-xs font-semibold uppercase tracking-wider ${t("text-gray-400", "text-gray-500")}`}>Policy Changes</span>
                  </div>
                  <div className="p-4">
                    <pre className="text-xs font-mono leading-relaxed rounded-xl p-4 overflow-auto max-h-80 bg-gray-950 text-emerald-300 border border-gray-800">
